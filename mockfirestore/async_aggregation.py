...
class AsyncAggregationQuery:
    def __init__(self, query: "AsyncQuery", field: Optional[Union[str, Iterable[str]]] = None,
    ...
    async def get(self, transaction=None) -> List[List[AggregationResult]]:
        docs = self._query.stream(transaction=transaction)
        if self._aggregation_type == "count":
            value = len([doc async for doc in docs])
            ...
            value = sum([doc._get_by_field_path(field_str) or 0 async for doc in docs])
        elif self._aggregation_type == "avg":
            if self._field is None:
                raise ValueError("Field must be specified for average aggregation.")
            field_str = ".".join(self._field) if isinstance(self._field, Iterable) else self._field
            values = [doc._get_by_field_path(field_str)
                      async for doc in docs if doc._get_by_field_path(field_str) is not None]
